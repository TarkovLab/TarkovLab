...
interface GraphQLQuest {
  ...
  turnin: number | null;
  gameId: string | null;
  require: QuestRequirements | null;
  unlocks: string[] | null;
  reputation: QuestReputation[] | null;
  objectives: QuestObjective[] | null;
}

class AppState {
  private quests: GraphQLQuest[] = [];
  private selectedQuestId: number | null = null;
  private searchQuery: string = "";
  private selectedTraderFilter: string = "all";
  ...
  private async loadQuestsFromAPI(): Promise<void> {
    const query = `
      query FetchQuests {
        quests {
          id
  ...
  // Filter logic
  private getFilteredQuests(): GraphQLQuest[] {
  ...
  private selectQuest(questId: number): void {
...
// ----------------------------------------------------------------
const GIVER_TO_ID: Record<number, string> = {
  0: "prapor",
  1: "therapist",
  2: "skier",
  3: "peacekeeper",
...
const ID_TO_GIVER: Record<string, number> = {
  "prapor": 0,
  "therapist": 1,
  "skier": 2,
  "peacekeeper": 3,
...
const GIVER_TO_NAME: Record<number, string> = {
  0: "Prapor",
  1: "Therapist",
  2: "Skier",
  3: "Peacekeeper",
...
function formatObjective(obj: any): string {
  ...
  if (typeof obj.target === "string" && obj.target.startsWith("[") && obj.target.endsWith("]")) {
    try {
      ...
      if (Array.isArray(targets)) {
        const names = targets.map((t: string) => ITEM_NAMES[t] || t);
